...
namespace Soft.Web.Infrastructure.Installation
{
    ...
    public class InstallationLocalizationService : IInstallationLocalizationService
    {
        ...
        /// </summary>
        private const string LanguageCookieName = "soft.installation.lang";
            ...
            if (httpContext.Request.UserLanguages != null)
            {
                ...
                if (!String.IsNullOrEmpty(userLanguage))
                {
                    ...
                    language = availableLanguages
                        .FirstOrDefault(
                            l => userLanguage.StartsWith(l.Code, StringComparison.InvariantCultureIgnoreCase));
                }
            }
            if (language != null)
                return language;
        ...
        public virtual IList<InstallationLanguage> GetAvailableLanguages()
        {
            if (_availableLanguages == null)
            {
                ...
                foreach (
                    var filePath in
                        Directory.EnumerateFiles(webHelper.MapPath("~/App_Data/Localization/Installation/"), "*.xml"))
                {
                    ...
                    foreach (XmlNode resNode in xmlDocument.SelectNodes(@"//Language/LocaleResource"))
                    {
                        ...
                        if (resNameAttribute == null)
                            throw new SoftException("All installation resources must have an attribute Name=\"Value\".");
                        var resourceName = resNameAttribute.Value.Trim();
                        if (string.IsNullOrEmpty(resourceName))
                            throw new SoftException("All installation resource attributes 'Name' must have a value.'");

                        if (resValueNode == null)
                            throw new SoftException("All installation resources must have an element \"Value\".");
